...
QUERY_ALIASES = {
    "бнс": ("буровая", "свая"),
    "жб": ("железобетон",),
    "жбк": ("железобетон", "конструкция"),
    "мк": ("металлоконструкция",),
    "ири": ("iri", "ровность"),
    "мгн": ("маломобильный", "доступность"),
    "тсодд": ("дорожный", "знак", "разметка", "светофор"),
}
...
TOPIC_MARKERS = {
    ...
    "traffic": ("знак", "размет", "светофор", "огражден", "одд", "тсодд", "переход"),
    ...
    "geotechnics": ("фундамент", "основан", "грунт", "свай", "свая", "землян", "котлован", "транше", "засыпк", "геолог"),
    "fire": ("пожар", "эвакуац", "огнестой", "противопожар", "дымоудал", "гидрант"),
    "buildings": ("жил", "обществен", "производствен", "помещен", "здан"),
    "engineering": ("вентиляц", "воздухообмен", "отоплен", "водоснаб", "канализац", "труб", "воздуховод", "радиатор", "электро", "газ", "котельн"),
    "finishes": ("кровл", "изоляц", "отделоч", "штукатур", "шпаклев", "малярн", "облицов", "плитк", "стяжк", "пол", "фасад", "корроз"),
    ...
    "accessibility": ("мгн", "маломобил", "инвалид", "коляск", "доступност", "пандус", "тактильн"),
    "physics": ("теплов", "утепл", "освещен", "освещенност", "шум", "климат", "энергоэффект"),
}
...
def analyze_query(text: str) -> QueryProfile:
    base_tokens = [token for token in tokenize(text) if token not in STOP_WORDS]
    alias_tokens = [alias for token in base_tokens for alias in QUERY_ALIASES.get(token, ())]
    tokens = tuple(dict.fromkeys(base_tokens + alias_tokens))
