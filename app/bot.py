...
WELCOME = (
    ...
    "Можно задавать обычным языком вопросы по дорогам, мостам, конструкциям, фундаментам, "
    "отделке, пожарной безопасности и инженерным системам. Бесплатный гибридный поиск "
    "понимает профессиональные термины, сокращения, склонения и частые опечатки."
)
...
async def help_message(message: Message) -> None:
    await message.answer(
        ...
        "• отклонение стальной колонны от разбивочной оси;\n"
        "• вертикальность сборной ЖБ колонны длиной 8 м;\n"
        "• допуск защитного слоя 20 мм при сечении 300 мм;\n"
        "• смещение кирпичной стены от разбивочной оси;\n"
        "• отметка дна котлована после окончательной разработки;\n"
        ...
        "Если в базе есть проверенное требование, ответ начинается с допуска, затем показывает условия, "
        "контроль, документ, пункт/таблицу, редакцию и официальную ссылку. Если текста пункта ещё нет, "
        "бот подбирает применимые документы и не "
        "придумывает численную норму."
    )
